...
public class Monster : MonoBehaviour {
	...
	private float mLat = 0, mLon = 0;

	public float getmLat{ get{return mLat;} }
	public float getmLon{ get{return mLon;} }
	void Start () {
		tm = GameObject.Find ("Map").GetComponent<TileManager> ();
	}

	public void Init(float tmLat, float tmLon){
		mLat = tmLat;
		mLon = tmLon;
		updatePosition ();
	}
	...
	public void updatePosition(){
		...
		Debug.Log ("monster-> "+mLat + " " + mLon);
		Debug.Log ("tm-> "+tm.getLat + " " + tm.getLon);
		geodeticOffsetInv (tm.getLat * Mathf.Deg2Rad, tm.getLon * Mathf.Deg2Rad, mLat * Mathf.Deg2Rad, mLon * Mathf.Deg2Rad, out x, out y);

		if ((mLat - tm.getLat) < 0 && (mLon - tm.getLon) > 0 || (mLat - tm.getLat) > 0 && (mLon - tm.getLon) < 0) {
			position = new Vector3 (x, 0.25f, y);
		} else if ((mLat - tm.getLat) == 0 && (mLon - tm.getLon) == 0) {
			//do nothing
		} else {
			position = new Vector3 (-x, 0.25f, -y);
		}
